...
const calculateMatchInsights = ({
  ...
  const normalizedVerifiedSkillSet = new Set(uniqueSkills(verifiedSkills).map(normalize));
  ...
  const verifiedMatchedCount = matchedSkills.filter((item) =>
    normalizedVerifiedSkillSet.has(normalize(item.skill))
  ).length;
  const verifiedBonus = Math.min(5, verifiedMatchedCount * 0.8);
