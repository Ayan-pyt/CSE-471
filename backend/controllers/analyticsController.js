...
const getMarketSkillTrends = async (req, res) => {
  try {
    ...
    const demandGap = requested.slice(0, 30).map((item) => ({
      skill: item.skill,
      demandCount: item.count,
      availableStudents: availability.get(item.skill) || 0,
      gap: item.count - (availability.get(item.skill) || 0),
    }));

    res.json({
      mostRequestedTechnicalSkills: requested.slice(0, 20),
      emergingSkillTrends: requested.slice(0, 30),
      departmentWiseSkillDemandGap: demandGap,
    });
  } catch (err) {
