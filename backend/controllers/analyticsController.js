...
const { syncAdzunaJobs } = require('../utils/jobMarketService');

const normalizeSkillValue = (value) => {
  if (typeof value === 'string') return value.trim().toLowerCase();
  if (value && typeof value === 'object') {
    const nested = value.skill || value.name || '';
...
const buildSkillDemandFromExternalJobs = (jobs = []) => {
  const demandInput = jobs.map((job) => ({
    requiredSkills: (job.skills || []).map((skill) => ({ skill })),
  }));

  return buildSkillDemand(demandInput);
};
...
const getMarketSkillTrends = async (req, res) => {
  try {
    const [externalJobs, internships, profiles] = await Promise.all([
      ExternalJobPost.find().lean(),
      Internship.find().lean(),
      StudentProfile.find().lean(),
    ]);

    const requestedFromExternal = buildSkillDemandFromExternalJobs(externalJobs);
    const requestedFromInternal = buildSkillDemand(internships);
    const requested = requestedFromExternal.length > 0 ? requestedFromExternal : requestedFromInternal;

    const source = requestedFromExternal.length > 0 ? 'external_rapidapi' : 'internal_internships';

    const departments = Array.from(new Set(profiles.map((profile) => profile.department || 'Unknown'))).sort();
    const departmentWiseSkillDemandGap = departments.map((department) => {
      const departmentProfiles = profiles.filter((profile) => (profile.department || 'Unknown') === department);
      const studentSkills = new Set(
        departmentProfiles
    ...
    res.json({
      source,
      mostRequestedTechnicalSkills: requested.slice(0, 20),
      emergingSkillTrends: requested.slice(0, 30),
      departmentWiseSkillDemandGap,
    });
  } catch (err) {
    res.status(500).json({ message: 'Failed to fetch market trends', error: err.message });
  }
};

const syncExternalMarketJobs = async (req, res) => {
  try {
    const {
    ...
    res.json({
      message: 'External market jobs synced successfully',
      ...result,
    });
  } catch (err) {
    res.status(500).json({ message: 'Failed to sync external market jobs', error: err.message });
  }
};

const getAdminSystemSnapshot = async (req, res) => {
  try {
    const [users, activeCompanies, pendingCompanies] = await Promise.all([
      User.countDocuments(),
      User.countDocuments({ role: 'company', isActive: true, approvalStatus: 'approved' }),
      User.countDocuments({ role: 'company', approvalStatus: 'pending' }),
    ]);
    ...
    res.status(500).json({ message: 'Failed to fetch snapshot', error: err.message });
  }
};
