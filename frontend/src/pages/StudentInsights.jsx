...
export default function StudentInsights() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const [t, m, v, f] = await Promise.allSettled([
      ...
      setTrends(t.status === 'fulfilled' ? t.value.data : null);
      setMarketTrends(m.status === 'fulfilled' ? m.value.data : null);
      setVerifiedSkills(v.status === 'fulfilled' ? (v.value.data || []) : []);
      setFeedback(f.status === 'fulfilled' ? (f.value.data || []) : []);
    } catch {
      // Any unexpected runtime error still falls back safely.
      setTrends(null);
