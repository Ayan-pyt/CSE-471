...
export default function CompanyInsights() {
  ...
  const [verifyForm, setVerifyForm] = useState({ studentId: '', skill: '', source: 'manual', note: '' });
  ...
  const verifySkill = async (e) => {
    ...
    setVerifyForm({ studentId: '', skill: '', source: 'manual', note: '' });
  };
              ...
              <form className="glass-card" onSubmit={verifySkill}>
                ...
                <div style={{ marginBottom: '10px' }}>
                  ...
                  <select className="form-input" required value={verifyForm.studentId} onChange={(e) => setVerifyForm({ ...verifyForm, studentId: e.target.value })}>
                    ...
                    {applications.map((a) => (
                      <option key={`vs_${a._id}`} value={a.studentId?._id}>{a.studentId?.name || 'Student'} ({a.studentId?.email || 'N/A'})</option>
                    ))}
                  </select>
                </div>
                <div style={{ marginBottom: '10px' }}>
                  <label className="form-label">Skill</label>
                  <input className="form-input" required value={verifyForm.skill} onChange={(e) => setVerifyForm({ ...verifyForm, skill: e.target.value })} />
                </div>
                <div style={{ marginBottom: '10px' }}>
                ...
                <button className="btn-primary" type="submit">Verify Skill</button>
              </form>
